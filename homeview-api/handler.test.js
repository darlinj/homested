...
describe("finding the customer", () => {
  ...
  it("finds the customer successfully", () => {
    ...
    return handler.findCustomer({ queryStringParameters: {searchTerm : " 088044 1801004390"}}).then((data) => {expect(JSON.parse(data.body).message).toEqual(jsonResult)});
  });
  ...
  it("gets the status info from HDM", () => {
    ...
    axios.mockResolvedValue({ data: apiResponse });
    const jsonResult = { result:"TEST_COMPLETED_LIVE", data: {key1: "value1", key2: "value2"}}
    return handler.diagnosticTest({ queryStringParameters: {searchTerm : " 088044 1801004390"}}).then((data) => {expect(JSON.parse(data.body).message).toEqual(jsonResult)});
  });
});
